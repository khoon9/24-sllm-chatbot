...
if "context_history_02" not in st.session_state:
    st.session_state["context_history_02"] = []

st.set_page_config(
    page_title="ko-gemma2-9B",
    page_icon="📃",
)
...
def save_message(message, role):
    ...
    st.session_state["context_history_02"].append({"message": message, "role": role})
...
"""
<bos><start_of_turn>system 당신은 분류기입니다. 제공된 응답이 미리 구두적 표현에 알맞은지 판단하세요. 다음 규칙과 분류 예시를 기반으로 응답을 “정상” 또는 “비정상”으로 분류하세요.


규칙:

1. 존댓말을 포함하면 "비정상"으로 응답하세요.
2. 오로지 “정상” 또는 “비정상”으로만 응답하세요.
...
st.markdown(
    """
rtzr/ko-gemma-2-9b-it(필터링 구조 적용)\n
...
if message:
    init_context_length = len(st.session_state["context_history_02"])
    send_message(message, "human")
    
    while True:
        st.session_state["context_history_02"] = st.session_state["context_history_02"][:init_context_length]

        context = "\n\n".join([item["role"]+": "+item["message"] for item in st.session_state["context_history_02"]])
        ...
        if init_context_length > 0:
            init_context_length -= 1
        ...
        formatted_prompt_02 = prompt_02.format(**chain_input_02)
